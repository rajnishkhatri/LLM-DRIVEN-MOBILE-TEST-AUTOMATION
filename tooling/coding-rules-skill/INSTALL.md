...
1. Create an architecture-test source set/module and copy the seed classes from
   `$O1/docs/coding-rules/archunit-seeds.md` into it.

   **Pick the artifact by JUnit major version, not by habit.** The suffix is the
   *engine*, and a mismatch does not fail loudly — the wrong engine simply
   discovers no `@ArchTest`, so the suite reports success having run nothing.

   | Your JUnit | Artifact | Notes |
   |---|---|---|
   | JUnit 5 | `com.tngtech.archunit:archunit-junit5` | |
   | JUnit 6 | `com.tngtech.archunit:archunit-junit6` | Spring Boot 4.x BOMs pin JUnit 6 |

   Maven (**the spine's build** — version 1.5.0, current as of 2026-08-09):
   ...
   Gradle equivalent: `testImplementation 'com.tngtech.archunit:archunit-junit6:1.5.0'`.
...
1. `./mvnw -q verify` (Maven) or `./gradlew test` — the ArchUnit suite runs and
