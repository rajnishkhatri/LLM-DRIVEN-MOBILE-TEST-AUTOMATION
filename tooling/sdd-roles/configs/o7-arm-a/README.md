...
single `"arm"` field changed to `"A"`** — nothing else differs, and
...
Role sequence resolved from the registry's `arms[]`: solo (1 role).
...
verbatim except `--config configs/o7-arm-a/kernel-config.json`.
...
`contract-lint validate configs/o7-arm-a --kernel kernel` → exit 0, 29/0;
