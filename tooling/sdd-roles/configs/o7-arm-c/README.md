# o7 KernelConfig — arm `C` (full-line)
...
single `"arm"` field changed to `"C"`** — nothing else differs, and
...
Role sequence resolved from the registry's `arms[]`: specifier → architect → coder → cleaner → hardener → qa (6 roles).
...
verbatim except `--config configs/o7-arm-c/kernel-config.json`.
...
`contract-lint validate configs/o7-arm-c --kernel kernel` → exit 0, 29/0;
